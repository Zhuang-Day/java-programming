...
public class AssociationRules_Apriori_Car_2 {

	public static void main(String[] args) {
		AssociationRules_Apriori_Car_2 miner = new AssociationRules_Apriori_Car_2();
		miner.runAssociator();
	}
		...
		try {
			...
			if(cars.length != 0) {
				...
				for(int i = 0; i < n; i++) {
					...
					// generate the text of a premise
					String text_p = getItemSetText(premise, associator.getInstancesNoClass());
					// generate the text of a conclusion
					String text_c = getItemSetText(conclusion, associator.getInstancesOnlyClass());
					
					// show the mined rule
					System.out.print("[" + (i+1) + "] ");
					System.out.print(text_p);
					System.out.print(" ==> ");
					System.out.print(text_c);
	...
	public String getItemSetText(ItemSet itemset, Instances instances) {
		...
		for (int i = 0; i < instances.numAttributes(); i++) {
			if (items[i] != -1) {
				...
				text.append(instances.attribute(i).value(items[i]) + ' ');
			}
		}
		text.append(itemset.counter());
